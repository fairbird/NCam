...
static struct PreloadedData g_preloaded = {
	.total_files = 0,
	.partition_count = 0,
	.indices = NULL,
	.last_update = 0
...
static struct ECMEntry *load_and_sort_entries(const char *filename, size_t *count) {
	...
	if (fd == -1) {
		cs_log("Error: %s - %s", filename, strerror(errno));
		return NULL;
	}

	if (fstat(fd, &sb) == -1) {
		close(fd);
		cs_log("Error: %s - Failed to get file size - %s", filename, strerror(errno));
		return NULL;
	}

	if (sb.st_size == 0 || sb.st_size % entry_size != 0) {
		close(fd);
		cs_log("Error: %s - Invalid file size or format", filename);
		return NULL;
	}

	*count = sb.st_size / entry_size;
	struct ECMEntry *entries = malloc(*count * sizeof(struct ECMEntry));
	if (!entries) {
		close(fd);
		return NULL;
	}

	uint8_t *temp_buffer = malloc(entry_size);
	if (!temp_buffer) {
		free(entries);
		close(fd);
		return NULL;
	...
	for (size_t i = 0; i < *count; i++) {
		ssize_t bytes_read = read(fd, temp_buffer, entry_size);
		if (bytes_read < 0 || (size_t)bytes_read != entry_size) {
			for (size_t j = 0; j < i; j++) free(entries[j].ecm);
			free(entries);
			free(temp_buffer);
			close(fd);
			return NULL;
...
static void preload_all_bin_files(const char *directory) {
	...
	for (int i = 0; i < MAX_CHANNELS * MAX_CHANNEL_FILES / PARTITION_SIZE; i++) {
	...
	if (!dir) {
		cs_log("Error opening bin folder %s: %s", directory, strerror(errno));
		return;
	}
	...
	g_preloaded.indices = malloc(total_files * sizeof(struct FileIndex));
	if (!g_preloaded.indices) {
		closedir(dir);
		return;
	}
	...
	while ((entry = readdir(dir)) != NULL) {
		...
		if (!parse_filename(entry->d_name, &caid, &srvid)) {
			cs_log("Warning: File %s doesn't match CAID@SRVID format, skipping", entry->d_name);
			continue;
		}
		...
		size_t count;
		struct ECMEntry *entries = load_and_sort_entries(fullpath, &count);
		if (entries && count > 0) {
			...
			cs_log("Loaded %s with %zu entries (CAID: %04X, SRVID: %04X)", 
				   entry->d_name, count, caid, srvid);
		}
...
static int32_t ecmbin_do_ecm(struct s_reader *UNUSED(rdr), const ECM_REQUEST *er, struct s_ecm_answer *ea) {
	...
	if (file_idx == (size_t)-1) {
		cs_log("No matching bin file found for CAID: %04X, SRVID: %04X", 
				  er->caid, er->srvid);
		return CS_ERROR;
	}
...
static void cleanup(void) {
	// Clean up partitions
	for (int i = 0; i < g_preloaded.partition_count; i++) {
		struct FilePartition *partition = &g_preloaded.partitions[i];
		pthread_rwlock_wrlock(&partition->lock);

		for (int j = 0; j < partition->file_count; j++) {
			struct BinFile *file = &partition->files[j];
			if (file->entries) {
				for (size_t k = 0; k < file->count; k++) {
...
static int32_t ecmbin_reader_init(struct s_reader *UNUSED(reader)) { preload_all_bin_files(cfg.bin_folder); return CR_OK; } 
static int32_t ecmbin_close(struct s_reader *UNUSED(reader)) { cs_log("Reader is shutting down"); cleanup(); return CR_OK; } 
...
static int32_t ecmbin_write_settings(struct s_reader *UNUSED(reader), struct s_cardreader_settings *UNUSED(s)) { return CR_OK; } 
static int32_t ecmbin_card_write(struct s_reader *UNUSED(pcsc_reader), const uint8_t *UNUSED(buf), uint8_t *UNUSED(cta_res), uint16_t *UNUSED(cta_lr), int32_t UNUSED(l)) { return CR_OK; } 
static int32_t ecmbin_set_protocol(struct s_reader *UNUSED(rdr), uint8_t *UNUSED(params), uint32_t *UNUSED(length), uint32_t UNUSED(len_request)) { return CR_OK; }
