...
static int32_t drecas_send_cmd(struct s_reader *reader, uint8_t *cmd, int32_t cmdlen, uint8_t *cta_res, uint16_t *p_cta_lr, uint8_t dest)
{
	// any command starts with this,
	// last byte is nr of bytes of the command that will be sent
	uint8_t startcmd[3] = { 0xDB, 0x00, 0x00 };
...
static int32_t drecas_MSP_command(struct s_reader *reader, const uint8_t *cmd, int32_t cmdlen, uint8_t *cta_res, uint16_t *p_cta_lr)
{
...
static int32_t drecas_set_provider_info(struct s_reader *reader)
{
	def_resp;
	int32_t i;
	uint8_t subscr[] = { 0x59, 0x14 }; // subscriptions
		...
		for(i = 0; i < 32; i++)
		{
			if(pbm[i] != 0xff)
			{
				...
				rdr_log(reader, "active package %i valid from %04i/%02i/%02i to %04i/%02i/%02i",
						i, startyear, startmonth, startday, endyear, endmonth, endday);
...
static int32_t drecas_card_init(struct s_reader *reader, ATR *newatr)
{
	...
	// discard first and last byte, last byte is always checksum, first is answer code
	int32_t hexlength = cta_res[5] - 2;
	...
	// copy first byte of unique address also in shared address, because we don't know what it is...
	memcpy(reader->sa[0], reader->hexserial + 2, 1);
