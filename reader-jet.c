...
static int32_t jet_resync_vendorkey(struct s_reader *reader)
{
	...
	memcpy(resync_vendorkey_cmd + 12, reader->boxkey, reader->boxkey_length);
...
static int32_t jet_card_init(struct s_reader *reader, ATR *newatr)
{
	...
	if((hist_size < 14) || (memcmp(hist, "FLASH ATR DVB", 13) && memcmp(hist, "DVN TECH", 8) != 0))
	{
		return ERROR;
	}
	...
	{
		memset(reader->boxkey, 0xEE, reader->boxkey_length);
	}
	...
	//pairing step1
	memcpy(pairing_cmd01 + 4, reader->boxkey, reader->boxkey_length);
	...
	//pairing step 2
	memcpy(pairing_cmd02 + 4, reader->boxkey, reader->boxkey_length);
	...
	if(cas_version <= 52)
	{
		for(i = 1; i <= 7; i++)
		{
			...
			cmd_buf[0] = 0x38;
			cmd_buf[4] = i;
			memcpy(confirm_box_cmd + 6, reader->boxkey, reader->boxkey_length);
	...
	else
	{
		...
		memcpy(cmd_buf + 4, reader->boxkey, reader->boxkey_length);
	...
	//unknow cmd 1
	memcpy(unknow_cmd1 + 7, reader->boxkey, reader->boxkey_length);
	jet_write_cmd_hold(reader, unknow_cmd1, sizeof(unknow_cmd1), 0x15, "unknow_cmd1");

	//update card serial
	get_serial_cmd[4] = 0x01;
	memcpy(get_serial_cmd + 5, reader->boxkey, reader->boxkey_length);
	...
	rdr_log_sensitive(reader, "type: jet, caid: %04X, serial: %llu, hex serial: %08llX, boxkey: %s",
					  ...
					  cs_hexdump(0, reader->boxkey, reader->boxkey_length, (char *)buf, sizeof(buf)));
...
static int32_t jet_do_ecm(struct s_reader *reader, const ECM_REQUEST *er, struct s_ecm_answer *ea)
{
	...
	memcpy(cmd + 4 + ecm_len, reader->boxkey, reader->boxkey_length);
...
static int32_t jet_do_emm(struct s_reader *reader, EMM_PACKET *ep)
{
	...
	memcpy(cmd + 4 + len, reader->boxkey, reader->boxkey_length);
...
static int32_t jet_card_info(struct s_reader *reader)
{
	...
	memcpy(get_entitlements_cmd1 + 6, reader->boxkey, reader->boxkey_length);
	...
	memcpy(get_entitlements_cmd2 + 7, reader->boxkey, reader->boxkey_length);
