...
*/
	...
	if(reader->rom[0] == 0x54 && reader->rom[1] == 0x33 && reader->rom[2] == 0x56 && reader->rom[3] == 0x56)
	{
		static const uint8_t ins42T3VV[5] = { 0xD1, 0x42, 0x00, 0x00, 0x25 };
		static const uint8_t payload42T3VV[0x25] = { 0x90, 0x23, 0x40, 0x01, 0xA4, 0xE7, 0x6F, 0xE3, 0xA3, 0x18, 0x4C, 0xE0, 0x1C, 0x76, 0x47, 0x69, 0xC1, 0x43, 0x87, 0x39, 0xA3, 0x93, 0xCC, 0x24, 0xAA, 0x3F, 0x78, 0x1A, 0x97, 0xB0, 0x80, 0xE7, 0xC5, 0xA6, 0x76, 0xE7, 0x16 };
		l = do_cmd(reader, ins42T3VV, payload42T3VV, NULL, cta_res);
		if(l < 0 || !status_ok(cta_res))
		{
			rdr_log(reader, "classD1 ins42T3VV: failed");
			return ERROR;
		}
	}
