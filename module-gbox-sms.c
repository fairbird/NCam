...
static void write_gsms_nack (struct s_client *cl, uint8_t inf)
{
	...
	if(inf)
	{
		fprintf(fhandle, "INFO: GSMS to all: Peer %04X (%s) was OFFLINE %s",
			peer->gbox.id, cl->reader->device,tsbuf);
	}
	else
	{
		fprintf(fhandle, "WARNING: Private GSMS to Peer %04X (%s) failed - was OFFLINE %s",
			peer->gbox.id, cl->reader->device,tsbuf);
	}
...
int gbox_direct_send_gsms(uint16_t boxid, uint8_t num, char *gsms)
{
	...
	switch(num)
	{
		...
		default:
			cs_log("ERROR unknown gsms protocol");
			return 0;
	}
